...
var adjectedTiles : Array = [Vector2(0, 0), Vector2(1, 0), Vector2(2, 0), Vector2(3, 0)]
...
func _physics_process(_delta):
	...
	if carTilePos in adjectedTilesPos and !adjectedTilePlaced and navAgent.is_target_reachable() and tileMap.get_cell_atlas_coords(0, carTilePos) not in houseTiles:
		...
		if houseCord.x < adjectedTilesPos[i].x:
			tileMap.set_cell(3, adjectedTilesPos[i], 7, adjectedTiles[3])
			tileMap.set_cell(5, houseCord, 2, navTiles[3])
		elif houseCord.x > adjectedTilesPos[i].x:
			tileMap.set_cell(3, adjectedTilesPos[i], 7, adjectedTiles[0])
			tileMap.set_cell(5, houseCord, 2, navTiles[0])
		elif houseCord.y < adjectedTilesPos[i].y:
			tileMap.set_cell(3, adjectedTilesPos[i], 7, adjectedTiles[2])
			tileMap.set_cell(5, houseCord, 2, navTiles[2])
		elif houseCord.y > adjectedTilesPos[i].y:
			tileMap.set_cell(3, adjectedTilesPos[i], 7, adjectedTiles[1])
	...
	if !navAgent.is_target_reachable() or (tileMap.get_cell_atlas_coords(0, nextTilePos) in houseTiles and nextTilePos != houseCord):
		bubble.tint_under = Color.RED
	else:
		bubble.tint_under = Color.WHITE
