...
@onready var skipButton : TextureRect = $"SkipTutorial"
...
func _process(_delta):
	...
	if Global.roadType == "remove":
		priceLabel.text = str("Doze")
	else:
		priceLabel.text = str("$", Global.currentPrice)
...
func tutorialText():
	match tutorialCount:
		...
		9:
			tutorialLabel.text = "You can rotate roads by pressing the R key"
		10:
